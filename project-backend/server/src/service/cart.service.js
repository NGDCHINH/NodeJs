...
const CartService = {
  ...
  incrementQuantity: (cart_id, product_id) => {
    return new Promise((resolve, reject) => {
      connection.query(
        "UPDATE cart_item SET quantity = quantity + 1 WHERE cart_id = ? AND product_id = ?",
        [cart_id, product_id],
        (err, result) => {
          if (err) {
            reject(err);
  ...
  getAllCartItem: (user_id) => {
    return new Promise((resolve, reject) => {
      connection.query(
        `SELECT products.product_id, products.product_name, products.price, cart_item.quantity FROM cart_item 
